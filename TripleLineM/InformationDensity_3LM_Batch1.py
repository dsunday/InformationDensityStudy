...
import time
import pickle
Param=np.loadtxt('TripleLineParameters_B1.txt')

def SimInt_ID3M(FITPAR):
    ...
    (Coord)= CD.ID3MCoordAssign(TPARs,SLD,Trapnumber,Pitch,X1)
...
def MCMCInit_ID3M(FITPAR,FITPARLB,FITPARUB,MCPAR):
    ...
    for i in range(int(MCPAR[0])):
        if i <MCPAR[3]: #reversed from matlab code assigns all chains below randomnumber as random chains
            for c in range(int(MCPAR[1])):
                ...
                (SimInt,Amplitude)=SimInt_ID3M(MCMCInit[i,:])
...
def MCMC_ID3M(MCMC_List):
    ...
    for step in np.arange(1,Stepnumber,1): 
        ...
        (SimPost,AmpPost)=SimInt_ID3M(Temp)
...
for SampleNumber in range(2): #len(Param[:,0])
    ...
    MCMCInitial=MCMCInit_ID3M(FITPAR,FITPARLB,FITPARUB,MCPAR)
    Acceptprob=0;
    ...
    if __name__ =='__main__':  
        ...
        F=pool.map(MCMC_ID3M,MCMC_List)
