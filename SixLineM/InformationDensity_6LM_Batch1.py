...
Param=np.loadtxt('SixLineParameters_B1.txt')
...
def MCMCInit_ID6M(FITPAR,FITPARLB,FITPARUB,MCPAR):
    ...
    for i in range(int(MCPAR[0])):
        if i <MCPAR[3]: #reversed from matlab code assigns all chains below randomnumber as random chains
            for c in range(int(MCPAR[1])):
                ...
                (SimInt,Amplitude)=SimInt_ID6M(MCMCInit[i,:])
...
def MCMC_ID6M(MCMC_List):
    ...
    for step in np.arange(1,Stepnumber,1): 
        ...
        (SimPost,AmpPost)=SimInt_ID6M(Temp)
...
for SampleNumber in range(2): #len(Param[:,0])
    ...
    MCMCInitial=MCMCInit_ID6M(FITPAR,FITPARLB,FITPARUB,MCPAR)
    Acceptprob=0;
