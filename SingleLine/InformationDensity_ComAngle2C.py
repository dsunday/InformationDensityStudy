...
import time
import pickle
Param=np.loadtxt('2AngleC_W12P32H200.txt')
...
for SampleNumber in range(len(Param[:,0])):   #len(Param[:,0])
    ...
    Qx = np.zeros([3,len(UQz)])
    Qz = np.zeros([3,len(UQz)])
    ...
    if __name__ =='__main__':  
        ...
        BR[SampleNumber,1]=100; BR[SampleNumber,2]=Pitch; BR[SampleNumber,3]=Angle; BR[SampleNumber,4:17]=UNCTAvg;
        np.savetxt('2AngleC_Com.csv',BR);
