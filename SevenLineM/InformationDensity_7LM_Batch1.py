...
Param=np.loadtxt('SevenLineParameters_B1.txt')

def SimInt_ID7M(FITPAR):
    ...
    (Coord)= CD.ID7MCoordAssign(TPARs,SLD,Trapnumber,Pitch,X1)
    ...
    for i in range(int(MCPAR[0])):
        if i <MCPAR[3]: #reversed from matlab code assigns all chains below randomnumber as random chains
            for c in range(int(MCPAR[1])):
                ...
                (SimInt,Amplitude)=SimInt_ID7M(MCMCInit[i,:])
...
def MCMC_ID6M(MCMC_List):
    ...
    for step in np.arange(1,Stepnumber,1): 
        ...
        (SimPost,AmpPost)=SimInt_ID7M(Temp)
        ...
        if ChiPost < ChiPrior:
            ...
            if np.random.random_sample() < MoveProb:
                ...
                SampledMatrixI[step,:]=SampledMatrix[step-1,:]
    ...
    (UNCT_Param)=Uncertainty2T(ReSampledMatrixI)
...
def Uncertainty2T(ReSampledMatrix):
        ...
        for PopNumber in range(len(ReSampledMatrix[:,0])):
            ...
            (CoordUnc)= CD.ID3MCoordAssign(TPARU,SLD,Trapnumber,Pitch,X1fit)
        ...
        return(vt,vct,RA,vtLower,vctLower,RAL,vtUpper,vctUpper,RAU,WidthAvg,HeightAvg,WidthStd,HeightStd,S1Avg,S1Std,LinePlot,InnerPlot,OuterPlot)

BR=np.zeros([len(Param[:,0]),19])
...
for SampleNumber in range(2): #len(Param[:,0])
    ...
    Coord=CD.ID7MCoordAssign(TPAR,SLD,Trapnumber,Pitch,X1)
    ...
    (Intensity,Amplitude)=SimInt_ID7M(FITPAR)
    ...
    MCPAR[5] = 1400 # stepbase
    MCPAR[6] = 1400 # steplength 
  
    
    MCMCInitial=MCMCInit_ID7M(FITPAR,FITPARLB,FITPARUB,MCPAR)
    Acceptprob=0;
    ...
    if __name__ =='__main__':  
        ...
        F=pool.map(MCMC_ID6M,MCMC_List)
        ...
        print(nonzerocount)
        UNCT=np.zeros([nonzerocount,15])
        nzc=-1        
        for i in range(int(MCPAR[0])):
            A=F[i]
            if A[0]!=0:
                nzc=nzc+1                
                UNCT[nzc,0:15]=A[0:15]
        UNCTAvg=np.average(UNCT,0)
        BR[SampleNumber,0]=W; BR[SampleNumber,1]=H; BR[SampleNumber,2]=Pitch; BR[SampleNumber,3]=Angle; BR[SampleNumber,4:19]=UNCTAvg;
        np.savetxt('SevenLine.csv',BR);
