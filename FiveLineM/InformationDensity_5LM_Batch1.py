...
Param=np.loadtxt('FiveLineParameters_B1.txt')

def SimInt_ID5M(FITPAR):
    ...
    (Coord)= CD.ID5MCoordAssign(TPARs,SLD,Trapnumber,Pitch,X1)
    ...
    for i in range(int(MCPAR[0])):
        if i <MCPAR[3]: #reversed from matlab code assigns all chains below randomnumber as random chains
            for c in range(int(MCPAR[1])):
                ...
                (SimInt,Amplitude)=SimInt_ID5M(MCMCInit[i,:])
...
def MCMC_ID5M(MCMC_List):
    ...
    for step in np.arange(1,Stepnumber,1): 
        ...
        (SimPost,AmpPost)=SimInt_ID5M(Temp)
...
for SampleNumber in range(2): #len(Param[:,0])
    ...
    Coord=CD.ID5MCoordAssign(TPAR,SLD,Trapnumber,Pitch,X1)
    ...
    (Intensity,Amplitude)=SimInt_ID5M(FITPAR)
    ...
    MCMCInitial=MCMCInit_ID5M(FITPAR,FITPARLB,FITPARUB,MCPAR)
    Acceptprob=0;
    ...
    if __name__ =='__main__':  
        ...
        F=pool.map(MCMC_ID5M,MCMC_List)
